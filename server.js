...
app.get('/api/ergobazaar/:category', async (req, res) => {
  const category = req.params.category;
  console.log(`🎮 ErgoBazaar data requested for: ${category}`);
  ...
  try {
    ...
    if (result.rows.length > 0) {
      console.log(`✅ ErgoBazaar ${category} data loaded from PostgreSQL`);
      res.setHeader('X-Data-Source', 'postgres');
      return res.json(result.rows[0].data);
    }
    ...
    const data = await fs.readFile(filePath, 'utf8');
    console.log(`✅ ErgoBazaar ${category} data loaded from JSON file`);
    res.setHeader('X-Data-Source', 'json-file');
    res.json(JSON.parse(data));
    
  } catch (err) {
    console.error(`❌ ErgoBazaar ${category} data loading error:`, err);
    res.status(500).json({ 
